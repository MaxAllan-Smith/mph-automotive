...
import { Car, CheckCircle2, ChevronDown, Search, SlidersHorizontal } from '@lucide/vue'
...
const availableModels = computed(
  () => vehicles.find((vehicle) => vehicle.make === make.value)?.models || [],
)
...
const selectedVehicleLabel = computed(() => {
  if (!detailsFound.value) return 'Add your registration or choose manually'

  return `${vehicleDetails.value.make} ${vehicleDetails.value.model} ${vehicleDetails.value.engine} ${vehicleDetails.value.year}`
})
const selectedVariant = computed(() => {
  return availableVariants.value.find((variant) => {
    return (
      variant.subModel === subModel.value &&
      variant.engine === engine.value &&
      variant.year === year.value
    )
  })
})
...
<template>
  <section class="bg-zinc-950 px-4 py-4 text-white sm:px-6 lg:px-8">
    <div class="mx-auto max-w-7xl">
      <button
        type="button"
        class="group flex w-full items-center justify-between gap-4 rounded-lg border border-white/10 bg-white/[0.06] p-4 text-left shadow-xl shadow-black/20 transition hover:border-red-400/60 hover:bg-white/[0.09]"
        :aria-expanded="isExpanded"
        aria-controls="vehicle-selection-panel"
        @click="isExpanded = !isExpanded"
      >
        <span class="flex min-w-0 items-center gap-3">
          <span
            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-md bg-red-600 text-white"
          >
            <Car class="h-5 w-5" aria-hidden="true" />
          </span>
          <span class="min-w-0">
            <span class="block text-xs font-black uppercase tracking-[0.22em] text-red-300"
              >Vehicle selection</span
            >
            <span class="mt-1 block truncate text-lg font-black text-white">{{
              selectedVehicleLabel
            }}</span>
          </span>
        </span>
        <span
          class="flex shrink-0 items-center gap-3 text-sm font-black uppercase tracking-wide text-red-100"
        >
          {{ isExpanded ? 'Hide' : 'Find parts for my vehicle' }}
          <ChevronDown
            :class="[
              'h-5 w-5 transition duration-300',
              isExpanded ? 'rotate-180 text-red-300' : 'text-white',
            ]"
            aria-hidden="true"
          />
        </span>
      ...
      >
        <div class="overflow-hidden">
          <div class="grid gap-5 lg:grid-cols-[0.95fr_1.25fr] lg:items-stretch">
            <div
              class="rounded-lg border border-white/10 bg-white/[0.04] p-5 shadow-2xl shadow-black/25"
            >
              <div class="mb-4 flex items-center gap-3">
                <span
                  class="flex h-10 w-10 items-center justify-center rounded-md bg-red-600 text-white"
                >
                  <Car class="h-5 w-5" aria-hidden="true" />
                </span>
                <div>
                  <p class="text-xs font-bold uppercase tracking-[0.24em] text-red-300">
                    Number plate lookup
                  </p>
                  <h2 class="text-2xl font-black tracking-tight">Enter your registration</h2>
                </div>
              </div>

              <form @submit="onSubmit" class="flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              <p class="mt-3 text-sm leading-6 text-zinc-300">Try MPH123, MPH24 or A3MPH.</p>
            </div>

            <div
              class="rounded-lg border border-white/10 bg-white p-5 text-zinc-950 shadow-2xl shadow-black/20"
            >
              <div class="mb-4 flex items-center justify-between gap-4">
                ...
                <SlidersHorizontal
                  class="hidden h-6 w-6 text-zinc-400 sm:block"
                  aria-hidden="true"
                />
              </div>
              ...
              <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                <div class="flex flex-col gap-1.5">
                  <label class="text-xs font-bold uppercase tracking-wide text-zinc-600"
                    >Make</label
                  >
                  <select
                    v-model="make"
                ...
                <div class="flex flex-col gap-1.5">
                  ...
                  >
                    ...
                    <option
                      ...
                      :key="variant.subModel"
                      :value="variant.subModel"
                    >
                      {{ variant.subModel }}
                    </option>
                  </select>
              ...
              <transition name="fade">
                <div
                  v-if="detailsFound"
                  class="mt-5 rounded-lg border border-red-100 bg-red-50 p-4"
                >
